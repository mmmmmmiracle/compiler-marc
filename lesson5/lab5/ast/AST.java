    ...
    public void nameAnalysis(SymTable symTab) {
        ...
        if(this.expNode instanceof IdNode){
            ...
            if(symInfo == null){
                ...
                SymInfo symInfoTemp = ( (StructInfo)symInfo ).getId().symInfo();
                structSymTab = ((StructDefInfo)symInfoTemp).getSymTable();
            }else{
...
public static class AssignNode extends ExpNode {
    ...
    public Type.AbstractType typeCheck(){
        Type.AbstractType lhsType = this.lhs.typeCheck();
        Type.AbstractType expType = this.rhs.typeCheck();
        Type.AbstractType retType = lhsType;
        // func1 = func2 is not allowed
        if(lhsType.isFnType() && expType.isFnType()){
            ErrMsg.fatal(this.lineNum, this.charNum, "Function assignment");
            retType = new Type.ErrorType();
        }
        //stuct1 = struct2 is not allowed
        ...
        //type missmatched
        else if((!lhsType.isErrorType() && !expType.isErrorType()) && !lhsType.equals(expType)){
            ErrMsg.fatal(lineNum, charNum, "Type missmatched");
            retType = new Type.ErrorType();
        }else if(lhsType.isErrorType() || expType.isErrorType()){
            retType = new Type.ErrorType();
        }else{
            ;
        }
...
public static class CallExpNode extends ExpNode {
    ...
    public Type.AbstractType typeCheck(){
        ...
        if( !type.isFnType() ){
            ErrMsg.fatal(this.idNode.lineNum, this.idNode.charNum, "Attempt to call a non-function");
        }else{
            fnInfo = (FnInfo)(this.idNode.symInfo());
            int actualNumParams = fnInfo.getNumParams();
            int getNumParams = this.expListNode.getExpNodeList().size();
