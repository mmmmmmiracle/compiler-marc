...
/**
 * This program is designed to test the SymInfo and SymTable classes.
 ...
 * 
 * Problems with SymInfo and/or SymTable methods are reported to stderr.
 * 
 */
public class Main {
    ...
    /**
     * Tests the SymInfo class.
     */
    private static void testSymInfo() {

        // test SymInfo methods using different types, including the empty 
        // string
        Type.AbstractType[] typeList = {new Type.BoolType(), new Type.IntType()};

        for (int i = 0; i < typeList.length; i++) {
            SymInfo sym = new SymInfo(typeList[i]);

            Type.AbstractType type = sym.getType();
            if (!type.equals(typeList[i])) {
                error("SymInfo.getType", 
                      "returns wrong value when type is " + typeList[i]);
            }

            type = sym.getType();
            if (!type.equals(typeList[i])) {
                error("SymInfo.toString", 
                      "returns wrong value when type is " + typeList[i]);
            }
        }
     ...
     * addDecl : add SymInfo into empty SymTable (i.e., has no HashMaps)
    ...
    private static void testExceptions() {
        ...
        try {
            ...
            symTab.removeScope();
            try {
                symTab.addDecl("name", new SymInfo(new Type.IntType()));
        ...
        symTab = new SymTable();
        try {
            symTab.addDecl(null, new SymInfo(new Type.IntType()));
            error("SymTable.addDecl", 
     ...
     * - add >1 SymInfo with unique names
     * - add SymInfo with duplicate name in SymTable with one HashMap and with >1
     *   HashMap (where name is in 1st HashMap) (should cause exception in 
     *   both cases)
     * - add SymInfo with name already in a HashMap but not the 1st one (should NOT
     *   cause an exception)
     */
    private static void testAddDecl() {
        SymTable symTab;
        String name1 = "aaa", name2 = "bbb", name3 = "ccc";
        SymInfo sym1 = new SymInfo(new Type.BoolType()), sym2 = new SymInfo(new Type.IntType());
    ...
    private static void testLookup() {
        SymInfo sym, oneSym = new SymInfo(new Type.IntType());
        ...
        try { 
            ...
            symTab.addScope();
            SymInfo localSym = new SymInfo(new Type.IntType());
            ...
            symTab = new SymTable();
            name = "b";
            sym = new SymInfo(new Type.IntType());
            for (int j=0; j<10; j++) {
                try {
    ...
    private static void testPrint() {
        ...
        try {
            symTab.print();
            try {
                symTab.addDecl("aaa", new SymInfo(new Type.BoolType()));
                symTab.addDecl("bbb", new SymInfo(new Type.IntType()));
                symTab.addScope();
                symTab.addDecl("ccc", new SymInfo(new Type.VoidType()));
                symTab.addScope();
                symTab.addDecl("ddd", new SymInfo(new Type.IntType()));
            } catch (Exception e) {
                error("SymTable.addDecl", 
