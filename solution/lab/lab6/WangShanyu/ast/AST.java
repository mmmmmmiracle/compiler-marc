...
public static class ExpListNode extends ASTnode {
     ...
     * typeCheck
     */
    public boolean typeCheck(List<Type.AbstractType> typeList) {
        boolean flag = true;
        Iterator<ExpNode> itExp = myExps.iterator();
        Iterator<Type.AbstractType> itType = typeList.iterator();
        while(itExp.hasNext()){
            ExpNode exp = itExp.next();
            Type.AbstractType type = exp.typeCheck();
            if(!type.equals(itType.next())){
                flag = false;
                ErrMsg.fatal(exp.getLineNum(), exp.getCharNum(), "Type of actual does not match type of formal");
            }
        }
        return flag;
    }
...
public static class IfStmtNode extends StmtNode {
     ...
     */
    public void typeCheck(Type.AbstractType retType) {
        Type.AbstractType expType = myExp.typeCheck();
...
public static class ReturnStmtNode extends StmtNode {
     ...
     */
    public void typeCheck(Type.AbstractType retType) {
        if(myExp == null && !retType.isVoidType())
...
public static class AssignNode extends ExpNode {
     ...
     */
    public Type.AbstractType typeCheck() {
        Type.AbstractType lhsType = myLhs.typeCheck();
        Type.AbstractType rhsType = myRhs.typeCheck();
...
public static class CallExpNode extends ExpNode {
     ...
     */
    public Type.AbstractType typeCheck() {
        Type.AbstractType idType = myId.typeCheck();
        if(idType.isErrorType() || !idType.isFnType()){
            ErrMsg.fatal(myId.getLineNum(), myId.getCharNum(), "Attempt to call a non-function");
            return new Type.ErrorType();
        }
        int numParam = ((FnInfo)(myId.info())).getNumParams();
        if((myExpList==null && numParam!=0) || numParam != myExpList.size()){
            ErrMsg.fatal(myId.getLineNum(), myId.getCharNum(), "Function call with wrong number of args");
            return new Type.ErrorType();
        }
        
        if(myExpList != null){
            List<Type.AbstractType> paramList =  ((FnInfo)(myId.info())).getParamTypes();
            if(!myExpList.typeCheck(paramList))
                return new Type.ErrorType();
        }
        return ((FnInfo)(myId.info())).getReturnType();
    }
...
public static class NotNode extends UnaryExpNode {
    ...
    public Type.AbstractType typeCheck() {
        ...
        if(type.isErrorType() || !type.isBoolType()){
            ErrMsg.fatal(myExp.getLineNum(), myExp.getCharNum(), "Logical operator applied to non-bool operand");
            return new Type.ErrorType();
        }
        return type;
...
public static abstract class ArithmeticExpNode extends BinaryExpNode {
     ...
     */
    public Type.AbstractType typeCheck() {
        Type.AbstractType lhsType = myExp1.typeCheck();
        Type.AbstractType rhsType = myExp2.typeCheck();
        boolean flag = false;
        if(!lhsType.isErrorType() && !lhsType.isIntType()){
            ErrMsg.fatal(myExp1.getLineNum(), myExp1.getCharNum(), "Arithmetic operator applied to non-numeric operand");
            flag = true;
        }
        if(!rhsType.isErrorType() && !rhsType.isIntType()){
